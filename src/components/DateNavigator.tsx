...
export const DateNavigator = React.memo(function DateNavigator({ onDateChange, availableDates }: DateNavigatorProps) {
  ...
  const datePresets = useMemo(() => {
    ...
    const availablePresets = visibleDates.map((dateStr, relativeIndex) => {
      ...
      let label = format(date, 'EEEE', { locale });
      if (diff === 0) label = t('time.today');
      if (diff === 1) label = t('time.yesterday');
      return {
        label: label,
        date: date,
        subLabel: format(date, 'MMM dd', { locale }),
        dateStr: dateStr,
        absoluteIndex: visibleStartIndex + relativeIndex, // 在 availableDates 中的绝对索引
      };
    });

    return [{ label: t('history.showAllHistory'), date: null, subLabel: t('history.showAllHistory'), dateStr: null, absoluteIndex: -1 }, ...availablePresets];
  }, [availableDates, visibleStartIndex, t, locale]);
