...
export function timeAgo(timestamp: number): string {
  ...
  if (interval >= 1) {
    ...
    return count === 1 ? t('time.yearsAgo', { count: 1 }) : t('time.yearsAgo', { count });
  }
  interval = seconds / 2592000;
  if (interval >= 1) {
    const count = Math.floor(interval);
    return count === 1 ? t('time.monthsAgo', { count: 1 }) : t('time.monthsAgo', { count });
  }
  interval = seconds / 86400;
  if (interval >= 1) {
    const count = Math.floor(interval);
    return count === 1 ? t('time.daysAgo', { count: 1 }) : t('time.daysAgo', { count });
  }
  interval = seconds / 3600;
  if (interval >= 1) {
    const count = Math.floor(interval);
    return count === 1 ? t('time.hoursAgo', { count: 1 }) : t('time.hoursAgo', { count });
  }
  interval = seconds / 60;
  if (interval >= 1) {
    const count = Math.floor(interval);
    return count === 1 ? t('time.minutesAgo', { count: 1 }) : t('time.minutesAgo', { count });
  }
