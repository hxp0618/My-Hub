...
const addHistoryDataRecursively = async (nodes: EnhancedBookmark[]): Promise<EnhancedBookmark[]> => {
  ...
  const fetchLastUsed = async (url: string) => {
    if (visitCache.has(url)) return visitCache.get(url)!;
    let lastVisit = 0;
    try {
      const visits = await chrome.history.getVisits({ url });
      lastVisit = visits?.[0]?.visitTime || 0;
    } catch {
      lastVisit = 0;
    }
    visitCache.set(url, lastVisit);
    return lastVisit;
  };

  const tasks: Array<() => Promise<void>> = [];

  const cloneNodes = (list: EnhancedBookmark[]): EnhancedBookmark[] =>
    list.map(node => {
      const cloned: EnhancedBookmark = { ...node };
      ...
      if (node.url) {
        tasks.push(async () => {
          cloned.dateLastUsed = await fetchLastUsed(node.url!);
        });
      }
  ...
  const clonedTree = cloneNodes(nodes);

  for (let i = 0; i < tasks.length; i += HISTORY_LOOKUP_BATCH) {
    await Promise.all(tasks.slice(i, i + HISTORY_LOOKUP_BATCH).map(task => task()));
  }
...
export function useBookmarks() {
  ...
  const [bookmarksWithHistory, setBookmarksWithHistory] = useState<EnhancedBookmark[] | null>(null);
  const [isLastUsedLoading, setIsLastUsedLoading] = useState(false);
  // 多选模式状态管理
  const [isMultiSelectMode, setIsMultiSelectMode] = useState(false);
  const [selectedBookmarkIds, setSelectedBookmarkIds] = useState<string[]>([]);
  const [isBulkUpdating, setIsBulkUpdating] = useState(false);
  const isBulkUpdatingRef = useRef(isBulkUpdating);
  // 位置保持: 存储最后删除的书签信息,用于在刷新后自动定位
  const [lastDeletedBookmarkId, setLastDeletedBookmarkId] = useState<string | null>(null);
  const lastDeletedBookmarkIdRef = useRef<string | null>(null);
  const [deletedBookmarkContext, setDeletedBookmarkContext] = useState<{
    deletedId: string;
    parentId: string;
    index: number;
  } | null>(null);
  ...
  useEffect(() => {
    lastDeletedBookmarkIdRef.current = lastDeletedBookmarkId;
  ...
  useEffect(() => {
    if (sortOrder.key !== 'dateLastUsed') {
      setIsLastUsedLoading(false);
      return;
    }

    if (isLastUsedLoading || bookmarksWithHistory || rawBookmarks.length === 0) {
      return;
    }

    let cancelled = false;
    const loadLastUsed = async () => {
      setIsLastUsedLoading(true);
      try {
        const historyEnriched = await addHistoryDataRecursively(rawBookmarks);
        if (!cancelled) {
          setBookmarksWithHistory(historyEnriched);
        }
      } catch (error) {
        if (!cancelled) {
  ...
  }, [sortOrder.key, rawBookmarks, bookmarksWithHistory, isLastUsedLoading]);

  useEffect(() => {
    const sourceBookmarks =
      sortOrder.key === 'dateLastUsed' && bookmarksWithHistory
        ? bookmarksWithHistory
        : rawBookmarks;
    const sorted = sortBookmarksRecursively(sourceBookmarks, sortOrder);
    setBookmarks(sorted);
  }, [rawBookmarks, bookmarksWithHistory, sortOrder]);
  ...
  const deleteBookmark = async (id: string) => {
    try {
      ...
      if (node[0]) {
        const parentId = node[0].parentId;
        const index = node[0].index || 0;

        setLastDeletedBookmarkId(id);
        setDeletedBookmarkContext({
          deletedId: id,
          parentId: parentId || '',
          index: index
        });
  ...
  const effectiveLoading = loading || (sortOrder.key === 'dateLastUsed' && isLastUsedLoading);
  ...
  return {
    ...
    clearLastDeletedBookmarkId: () => {
      setLastDeletedBookmarkId(null);
      setDeletedBookmarkContext(null);
    },
  };
}
