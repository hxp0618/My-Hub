  ...
  return (
    <div className="space-y-6">
        ...
        {stats.map(({ label, value, icon, color }, index) => (
          ...
          >
            ...
            <div className="flex items-start gap-4">
              {/* 图标容器 */}
              <div className={`w-12 h-12 flex items-center justify-center ${color} border-2 border-[color:var(--nb-border)] shadow-[3px_3px_0px_0px_var(--nb-shadow-color)] flex-shrink-0`}>
      ...
      {statistics?.topTags && statistics.topTags.length > 0 && (
        <div className="nb-card-static p-6 shadow-[6px_6px_0px_0px_var(--nb-shadow-color)] relative overflow-hidden">
          {/* 装饰性元素 */}
          <div className="absolute -bottom-4 -right-4 w-20 h-20 bg-[color:var(--nb-accent-green)]/20 border-2 border-[color:var(--nb-border)]/30 nb-sticker-3 pointer-events-none" style={{ borderRadius: '30% 70% 70% 30% / 30% 30% 70% 70%' }}></div>
          
          <div className="flex items-center gap-3 mb-5">
            <div className="w-10 h-10 flex items-center justify-center bg-[color:var(--nb-accent-green)] border-2 border-[color:var(--nb-border)] shadow-[3px_3px_0px_0px_var(--nb-shadow-color)]">
          ...
          <div className="flex flex-wrap gap-3 relative z-10">
            {statistics.topTags.map((tag, index) => (
              <span
                key={tag.name}
                className={getTagClassName(index, 'shadow-[3px_3px_0px_0px_var(--nb-shadow-color)] hover:shadow-[1px_1px_0px_0px_var(--nb-shadow-color)] hover:translate-x-[2px] hover:translate-y-[2px] transition-all duration-100 cursor-default')}
              >
