...
import { getNextExecutionTime } from '../../utils/simpleCronParser';
  ...
  try {
    ...
    const updatedTasks = tasks.map(t => {
      ...
      if (t.type === 'one-time') {
        ...
        if (t.cronExpression) {
          const nextTime = getNextExecutionTime(t.cronExpression);
          if (nextTime) {
    ...
    for (const task of tasks) {
      ...
      if (!task.nextExecutionTime || task.nextExecutionTime < now) {
        if (task.type === 'one-time') {
          ...
          // 周期性任务需要重新计算下次执行时间
          const nextTime = getNextExecutionTime(task.cronExpression);
          if (nextTime) {
